...
class OneShot(PromptStrategy):
    ...
    def generate_prompt_array(self, prompts, document_type, length):
        ...
        for i in  tqdm.tqdm(range(len(prompt_list))):
              ...
              one_shot_prompts.append( f"The following is an excerpt from a(n) {document_type}, followed by a completion of that excerpt. Please complete the unfinished {document_type} excerpt. {example[0]} => {example[1]} .{prompt[0]} =>")
...
class FewShot(PromptStrategy):
    ...
    def generate_prompt_array(self, prompts, document_type, length):
        ...
        for i in  tqdm.tqdm(range(len(prompt_list))):
              ...
              prompt = divide_data(length,prompt_list[i])
              few_shot_prompts.append( f"The following is an excerpt from a(n) {document_type}, followed by a completion of that excerpt. Please complete the unfinished {document_type} excerpt. {example} .{prompt[0]} =>")
