...
if __name__ == "__main__":
    ...
    name = "PubMed"
    group =data[name]["pythia-1.4b"]
    for prompt_type in data[name]["pythia-1.4b"]:
                df = pd.DataFrame(group["fewshot"], columns=['Entry'])
                cleaner = CleanData(CloseVocab())
                clean_data = cleaner.cleanTheArray(df['Entry'])
                cleaner.saveData(clean_data,name+"_" +"pythia-1.4b"+"_"+"fewshot"+ "_Close")
