###
title: "figure-F1-the-logistic-model-of-debowski"
...
def fit_g_paper(N, V):
    ...
    # initial guesses:
    # beta ~ slope on log-log (middle region)
    lo = max(1, x.size // 10)
    hi = max(lo + 3, x.size * 8 // 10)
    b_guess = float(np.polyfit(np.log(x[lo:hi]), np.log(y[lo:hi]), 1)[0])
    b_guess = min(max(b_guess, 0.05), 0.95)

    # gamma: gentle curvature as a start
    g_guess = 0.5

    # alpha: shift so that bend sits around a mid-quantile of N
    ...
    alpha, beta, gamma = map(float, popt)   # <-- only 3 params

    # now compute R^2 separately
