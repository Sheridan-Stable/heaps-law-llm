...
basic_dat <- rawdata |>
  select(-c(file_name, mean, sd, vocab_size, total_words, alpha, singleton_count, r, model_size, prompt)) |> # Drop irrelevant columns
  mutate(
...
for (v in vocabs) {
  ...
  p <- ggplot(plot_data, aes(x = model, y = beta_avg, color = corpus)) +
    geom_point(size = 2) +
    geom_errorbar(
      aes(ymin = beta_avg - 0.25 * beta_sd,
          ymax = beta_avg + 0.25 * beta_sd),
      width = 0.5
    ) +
    scale_color_manual(
      values = corpus_palette,
      breaks = c("wiki", "pubmed", "hn", "book2"),
      labels = c("Wiki", "Pubmed", "Hacker News", "Book2")
    ) +
    labs(
      title = paste(toupper(substr(v, 1, 1)), substr(v, 2, nchar(v)),
                    " vocabulary setting", sep = ""),
      x = "",
      y = "Mean beta",
      color = "Corpus"
    ) +
